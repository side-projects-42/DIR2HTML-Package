...
describe("api", function () {
  describe(".render(options, callback)", function () {
    ...
    it("should not generate source map when not requested", function (done) {
      sass.render(
        ...
        function (error, result) {
          assert.strictEqual(
            result.hasOwnProperty("map"),
            false,
            "result has a map property"
          );
          done();
        }
      );
    ...
    it("should add cwd to the front on include paths", function (done) {
      ...
      var expected = read(
        fixture("cwd-include-path/expected.css"),
        "utf8"
      ).trim();
      var cwd = process.cwd();
    ...
    it("should check SASS_PATH in the specified order", function (done) {
      ...
      var expectedRed = read(
        fixture("sass-path/expected-red.css"),
        "utf8"
      ).trim();
      var expectedOrange = read(
        fixture("sass-path/expected-orange.css"),
        "utf8"
      ).trim();

      var envIncludes = [fixture("sass-path/red"), fixture("sass-path/orange")];
    ...
    it("should prefer include path over SASS_PATH", function (done) {
      ...
      var envIncludes = [fixture("sass-path/red")];
      process.env.SASS_PATH = envIncludes.join(path.delimiter);
  ...
  describe(".render(importer)", function () {
    ...
    it("should respect the order of chained imports when using custom importers and one file is custom imported and the other is not.", function (done) {
      sass.render(
        ...
        function (err, data) {
          ...
          assert.equal(data.css.toString().trim(), 'body {\n  color: "red"; }');
      ...
      sass.render(
        {
          ...
          importer: function (url, /* jshint unused:false */ prev, done) {
              ...
              file: path.resolve(
                path.dirname(fixture("include-files/index.scss")),
                url + (path.extname(url) ? "" : ".scss")
              ),
            });
          },
        },
    ...
    it("should throw exception when importer returns an invalid value", function (done) {
      sass.render(
        ...
        function (error) {
          assert(
            /returned value of `contents` must be a string/.test(error.message)
          );
          done();
        }
      );
  ...
  describe(".render(functions)", function () {
    ...
    it("should properly convert boolean when calling custom functions", function (done) {
      sass.render(
        {
          data:
            "div { color: if(foo(true, false), #fff, #000);" +
    ...
    it("should properly convert maps when calling custom functions", function (done) {
      sass.render(
        {
          data:
            "$test-map: foo((abc: 123, #def: true)); div { color: if(map-has-key($test-map, hello), #fff, #000); }" +
    ...
    describe("should properly bubble up errors from sass color constructor", function () {
      it("four booleans", function (done) {
        sass.render(
          ...
          function (error) {
            assert.ok(
              /Constructor arguments should be numbers exclusively/.test(
                error.message
              )
            );
            done();
      ...
      it("two arguments", function (done) {
        sass.render(
          ...
          function (error) {
            assert.ok(
              /Constructor should be invoked with either 0, 1, 3 or 4 arguments/.test(
                error.message
              )
            );
            done();
    ...
    it("should fail when trying to set a bare number as the List item", function (done) {
      sass.render(
        ...
        function (error) {
          assert.ok(
            /Supplied value should be a SassValue object/.test(error.message)
          );
          done();
        }
      );
    ...
    it("should fail when trying to set a bare Object as the List item", function (done) {
      sass.render(
        ...
        function (error) {
          assert.ok(
            /A SassValue is expected as the list item/.test(error.message)
          );
          done();
        }
      );
    ...
    it("should fail when trying to set a bare Object as the Map value", function (done) {
      sass.render(
        ...
        function (error) {
          assert.ok(
            /A SassValue is expected as a map value/.test(error.message)
          );
          done();
        }
      );
  ...
  describe(".renderSync({stats: {}})", function () {
    ...
    it("should contain the given entry file", function (done) {
      assert.equal(
        result.stats.entry,
        resolveFixture("include-files/index.scss")
      );
      done();
    });
