...
var initialize = function (inputCss, options) {
  ...
  if (exists(testCase.optionsPath)) {
    options = mergeWith(
      assign({}, options),
      readYaml.sync(testCase.optionsPath),
      customizer
    );
  }
  testCase.includePaths = [folder, join(folder, "sub")];
  testCase.precision = parseFloat(options[":precision"]) || 5;
  testCase.outputStyle = options[":output_style"]
    ? options[":output_style"].replace(":", "")
    : "nested";
  testCase.todo =
    options[":todo"] !== undefined &&
    options[":todo"] !== null &&
    options[":todo"].indexOf(impl) !== -1;
  testCase.only =
    options[":only_on"] !== undefined &&
    options[":only_on"] !== null &&
    options[":only_on"];
  testCase.warningTodo =
    options[":warning_todo"] !== undefined &&
    options[":warning_todo"] !== null &&
    options[":warning_todo"].indexOf(impl) !== -1;
  ...
  testCase.shouldFail =
    exists(testCase.statusPath) &&
    !fs.statSync(testCase.statusPath).isDirectory();
  testCase.verifyStderr =
    exists(testCase.errorPath) &&
    !fs.statSync(testCase.errorPath).isDirectory();
  return testCase;
};
        ...
        function (error, result) {
          if (test.shouldFail) {
            ...
            if (error === null) {
              ...
              assert.ok(
                expectedError,
                "Expected some sort of warning, but found none"
              );
            } else {
...
var executeSuite = function (suite, tests) {
  ...
  if (exists(optionsFile)) {
    suite.options = mergeWith(
      assign({}, suite.options),
      readYaml.sync(optionsFile),
      customizer
    );
  }
