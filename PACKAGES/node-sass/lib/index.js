...
function getSourceMap(options) {
  ...
  return sourceMap && typeof sourceMap === "string"
    ? path.resolve(sourceMap)
    : null;
}
  ...
  if (functions) {
    ...
    Object.keys(functions).forEach(function (subject) {
      ...
      options.functions[cb.signature] = function () {
        ...
        var result = tryCallback(
          cb.callback.bind(options.context),
          args.concat(done)
        );
...
function processSassDeprecationMessage() {
  console.log(
    "Deprecation warning: `process.sass` is an undocumented internal that will be removed in future versions of Node Sass."
  );
}
