...
test("\nreading root using glob filter", function (t) {
  ...
  t.test('\n# ["root_dir1", "*dir1_subdir1"]', function (t) {
    t.plan(1);
    readdirp(
      opts({ directoryFilter: ["root_dir1", "*dir1_subdir1"] }),
      function (err, res) {
  ...
  t.test(
    '\n# leading and trailing spaces: [" *.ext1", "*.ext3 "]',
    function (t) {
      t.plan(1);
      readdirp(
        opts({ fileFilter: [" *.ext1", "*.ext3 "] }),
        function (err, res) {
          t.equals(
            res.files.length,
            ext1Files + ext3Files,
  ...
  t.test("\n# ** glob pattern", function (t) {
    ...
    readdirp(opts({ fileFilter: "**/*.ext1" }), function (err, res) {
      t.equals(
        res.files.length,
        ext1Files,
        "ignores ** in **/*.ext1 -> only *.ext1 files"
      );
      t.end();
    });
  });
...
test("\n\nreading root using function filter", function (t) {
  ...
  t.test('\n# directory filter -> "name has length 9"', function (t) {
    ...
    readdirp(
      ...
      function (err, res) {
        t.equals(
          res.directories.length,
          nameHasLength9Dirs,
          "all all dirs with name length 9"
        );
        t.end();
      }
    );
  ...
  t.test("\n# depth 1", function (t) {
    ...
    readdirp(opts({ depth: 1 }), function (err, res) {
      t.equals(
        res.files.length,
        depth1Files,
        "does not return files at depth 2"
      );
    });
  });
});
  ...
  t.test("\n# depth 0", function (t) {
    ...
    readdirp(opts({ depth: 0 }), function (err, res) {
      t.equals(
        res.files.length,
        depth0Files,
        "does not return files at depth 0"
      );
    });
  });
});
  ...
  readdirp(
    ...
    function (err, res) {
      t.equals(
        ...
        res.files.length,
        "calls back for each file processed"
      );
      t.deepEquals(
        processedFiles.map(pluckName).sort(),
        res.files.map(pluckName).sort(),
        "same file names"
      );
      t.end();
    }
  ...
  opts.forEach(function (op) {
    ...
    t.test("\n" + util.inspect(op), function (t) {
      ...
      readdirp(op, function (err, res) {
        ...
        t.equals(
          res.files[0].path,
          path.join(op.prefix, expected.path),
          "correct path"
        );
      });
