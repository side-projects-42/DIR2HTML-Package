...
module.exports = function (grunt) {
  grunt.registerMultiTask(
    "to_html",
    "Generate a project homepage from a directory structure",
    function () {
      ...
      if (options.template) {
        ...
        template = grunt.file.read(
          __dirname + "/templates/html-structure.jade"
        );
      } else {
      ...
      this.files.forEach(function (f) {
        ...
        f.src.forEach(
          buildDataStructure.bind(this, options, directoryStructure.modules)
        );
  ...
  function buildDataStructure(options, directoryStructure, filepath) {
    var splitPaths = getDirectoryInFilepath(
    ...
    if (options.useFileNameAsTitle) {
      ...
      directories = splitPaths
        .slice(0, filenameIndex)
        .concat(filenameWithoutExtension);
    } else {
      directories = splitPaths.slice(0, filenameIndex);
    }

    addFileLocationToDirectoryStructure(
      directoryStructure,
      directories,
      filepath,
      0
  ...
  function addFileLocationToDirectoryStructure(
    directoryStructure,
    directories,
    filepath,
    level
  ) {
    var directoryLocation = getDirectoryInFilepath(
      directories[0],
      filepath,
      false
    );
    ...
    if (fs.existsSync(pathToJSON)) {
      fileLocationData = grunt.util._.extend(
        fileLocationData,
        grunt.file.readJSON(pathToJSON)
      );
    }

    if (directories.length === 1) {
      directoryStructure.push(
        grunt.util._.extend(fileLocationData, {
          path: filepath,
        })
      );
    } else {
      // does this folder already exist in our directoryStructure
      var folder = findMatchingFolderInDirectoryStructure(
        directoryStructure,
        directories[0]
      );
  ...
  function findMatchingFolderInDirectoryStructure(structure, directoryName) {
    if (!structure || !structure[0]) {
      ...
      return findMatchingFolderInDirectoryStructure(
        structure.slice(1),
        directoryName
      );
    }
  ...
  function getTitle(directoryName) {
    ...
    var splitParentFinal =
      splitParent.length > 1 ? splitParent : directoryName.split("-");
