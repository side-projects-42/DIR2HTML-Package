...
exports.to_html = {
  default_options: function (test) {
    ...
    test.equal(
      actual,
      expected,
      "should create a basic html template from a simple directory structure"
    );
    ...
    test.equal(
      actual2,
      expected2,
      "should create a nested html structure corresponding to the directory structure"
    );
  ...
  module_directory_option: function (test) {
    ...
    test.equal(
      actual,
      expected,
      "should be able to handle a different module directory option"
    );
  ...
  generate_page_option: function (test) {
    ...
    test.equal(
      actual,
      expected,
      'should generate a full html page if the "generatePage" option is set to true'
    );
  ...
  template_option: function (test) {
    ...
    test.equal(
      actual,
      expected,
      'should use the template provided in the "options" object'
    );
  ...
  json_data: function (test) {
    ...
    test.equal(
      actual,
      expected,
      'should read a "data.json" file if it exists in the folder and render that data into the template'
    );
    ...
    test.equal(
      actual2,
      expected2,
      "should expose additional data to the template if it exists in the json data"
    );
  ...
  handlebars_option: function (test) {
    ...
    var template = handlebars.compile(
      grunt.file.read("tasks/templates/html-structure.hbs")
    );
      ...
      modules: [
        {
          title: "Data title",
          description:
            "This is a folder that contains some json data for the folder location",
    ...
    test.equal(
      actual,
      expected,
      "should use handlebars as the templating language if specified in the options"
    );
  ...
  title_option: function (test) {
    ...
    test.equal(
      actual,
      expected,
      'should use a different title if the "title" option is set'
    );
  ...
  stylesheet_option: function (test) {
    ...
    test.equal(
      actual,
      expected,
      'should use a stylesheet if the "stylesheet" option is set'
    );
  ...
  use_filename_as_title_option: function (test) {
    ...
    test.equal(
      actual,
      expected,
      "should be able to use filename as a title instead"
    );
  ...
  dashes_for_folder_name: function (test) {
    ...
    test.equal(
      actual,
      expected,
      "should be able to handle dashes in folder name"
    );
