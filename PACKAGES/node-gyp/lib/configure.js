...
exports.usage =
  "Generates " +
  (win ? "MSVC project files" : "a Makefile") +
...
function configure(gyp, argv, callback) {
  ...
  function getNodeDir() {
    ...
    if (gyp.opts.nodedir) {
      ...
      if (!release.semver) {
        ...
        return callback(
          new Error("Invalid version number: " + release.version)
        );
      }
  ...
  function createConfigFile(err, vsSetup) {
    ...
    if (vsSetup) {
      ...
      variables["msbuild_path"] = path.join(
        vsSetup.path,
        "MSBuild",
        "15.0",
        "Bin",
  ...
  function runGyp(err) {
    ...
    if (process.platform === "aix" || process.platform === "os390") {
      ...
      if (node_exp_file !== undefined) {
        ...
        var msg = msgFormat(
          "Could not find node.%s file in %s",
          ext,
          node_root_dir
        );
    ...
    fs.stat(common_gypi, function (err, stat) {
      ...
      argv.push(
        "-Dnode_engine=" + (gyp.opts.node_engine || process.jsEngine || "v8")
      );
      argv.push("--depth=.");
