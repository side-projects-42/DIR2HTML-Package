...
function install(fs, gyp, argv, callback) {
  ...
  if (semver.lt(release.version, "0.8.0")) {
    return callback(
      new Error(
        "Minimum target version is `0.8.0` or greater. Got: " + release.version
      )
    );
  }
  ...
  if (release.semver.prerelease[0] === "pre") {
    ...
    if (gyp.opts.nodedir) {
      log.verbose(
        "--nodedir flag was passed; skipping install",
        gyp.opts.nodedir
      );
      callback();
    } else {
    ...
    mkdir(devDir, function (err, created) {
      ...
      req.on("response", function (res) {
        if (res.statusCode !== 200) {
          ...
          cb(
            new Error(
              res.statusCode + " response downloading " + release.tarballUrl
            )
          );
          return;
        }
      ...
      function afterTarball() {
        ...
        function deref(err) {
          ...
          if (!async) {
            log.verbose(
              "download contents checksum",
              JSON.stringify(contentShasums)
            );
      ...
      function downloadShasums(done) {
        log.verbose(
          "check download content checksum, need to download `SHASUMS256.txt`..."
        );
        ...
        req.on("response", function (res) {
          if (res.statusCode !== 200) {
              ...
              new Error(res.statusCode + " status code downloading checksum")
            );
            return;
          }
      ...
      function downloadNodeLib(done) {
        log.verbose(
          "on Windows; need to download `" + release.name + ".lib`..."
        );
        ...
        mkdir(dir32, function (err) {
          ...
          log.verbose(
            "streaming 32-bit " + release.name + ".lib to:",
            libPath32
          );
          ...
          req.on("response", function (res) {
            if (res.statusCode !== 200) {
                ...
                new Error(
                  res.statusCode +
                    " status code downloading 32-bit " +
                    release.name +
                    ".lib"
                )
              );
        ...
        mkdir(dir64, function (err) {
          if (err) return done(err);
          log.verbose(
            "streaming 64-bit " + release.name + ".lib to:",
            libPath64
          );
          ...
          req.on("response", function (res) {
            if (res.statusCode !== 200) {
                ...
                new Error(
                  res.statusCode +
                    " status code downloading 64-bit " +
                    release.name +
                    ".lib"
                )
              );
  ...
  function eaccesFallback(err) {
    ...
    log.warn(
      "EACCES",
      'user "%s" does not have permission to access the dev dir "%s"',
      osenv.user(),
      devDir
    );
    log.warn(
      "EACCES",
      'attempting to reinstall using temporary dev dir "%s"',
      gyp.devDir
    );
...
function download(gyp, env, url) {
  ...
  var requestOpts = {
    ...
    headers: {
      "User-Agent":
        "node-gyp v" + gyp.version + " (node " + process.version + ")",
    },
  };
  ...
  var proxyUrl =
    gyp.opts.proxy || env.http_proxy || env.HTTP_PROXY || env.npm_config_proxy;
  if (proxyUrl) {
    if (/^https?:\/\//i.test(proxyUrl)) {
      ...
      log.warn(
        "download",
        'ignoring invalid "proxy" config setting: "%s"',
        proxyUrl
      );
    }
  }
