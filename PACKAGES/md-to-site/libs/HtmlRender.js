...
class HtmlRender {
  ...
  selectIndexPage(docs, indexDoc = null) {
    ...
    for (var i = 0; i < docs.length; i++) {
      if (
        indexDoc &&
        (indexDoc == docs[i].name.toLowerCase() ||
          indexDoc == docs[i].file.toLowerCase())
      ) {
        candidateIndex = i; // the candidate index found: the name matches with indexDoc
        break;
      } else if (
        !docs[i].parentDir &&
        ("readme" == docs[i].name.toLowerCase() ||
          "readme.md" == docs[i].file.toLowerCase())
      ) {
  ...
  copyAssetsToDestinationDir(target) {
    ...
    Utils.copyFileFromPackageDir(
      "/../assets/css/style.css",
      target + "css/style.css"
    );
    Utils.copyFileFromPackageDir(
      "/../assets/css/images/menu-white.svg",
      target + "css/images/menu-white.svg"
    );
    Utils.copyFileFromPackageDir(
      "/../assets/css/images/close-black.svg",
      target + "css/images/close-black.svg"
    );
    Utils.copyFileFromPackageDir(
      "/../assets/css/images/search-black.svg",
      target + "css/images/search-black.svg"
    );
    Utils.copyFileFromPackageDir(
