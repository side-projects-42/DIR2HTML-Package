...
groups.push([
  ...
  [
    "([\\s\\(\\)])([^\\s\\(\\)]+)->((?:",
    ["Boolean", "Number", "Integer", "Uint32", "Int32"].join("|"),
    ")Value)\\(",
  ].join(""),
]);
...
groups.push([
  ...
  [
    "(",
    ["NanDisposePersistent", "NanObjectWrapHandle"].join("|"),
    ")\\s*\\(\\s*([^\\s\\)]+)",
  ].join(""),
]);
...
function replace() {
  ...
  switch (arguments[groups[4][0]]) {
    ...
    case "SetIndexedPropertiesToPixelData":
      return arguments[groups[4][0] - 1]
        ? arguments[0]
        : [warning1, arguments[0]].join("");
    default:
  }
  ...
  switch (arguments[groups[6][0]]) {
    ...
    case "Uint32":
      return [
        arguments[groups[6][0] - 2],
        "NanTo<v8::",
        arguments[groups[6][0]],
        ">(",
        arguments[groups[6][0] - 1],
      ].join("");
    default:
  }
  ...
  switch (arguments[groups[7][0]]) {
    case "BooleanValue":
      return [
        arguments[groups[7][0] - 2],
        "NanTo<bool>(",
        arguments[groups[7][0] - 1],
      ].join("");
    case "Int32Value":
      return [
        arguments[groups[7][0] - 2],
  ...
  if (arguments[groups[8][0]] === "NAN_WEAK_CALLBACK") {
    return [
      "template<typename T>\nvoid ",
      arguments[groups[8][0] + 1],
      "(const NanWeakCallbackInfo<T> &data)",
    ].join("");
  }
  ...
  switch (arguments[groups[11][0]]) {
    ...
    case "ToDetailString":
      return [
        arguments[groups[11][0] - 2],
        "Nan",
        arguments[groups[11][0]],
        "(",
        arguments[groups[11][0] - 1],
      ].join("");
...
function processFile(file) {
  fs.readFile(file, { encoding: "utf8" }, function (err, data) {
    ...
    fs.writeFile(
      file,
      data.replace(master, replace).replace(master, replace),
      function (err) {
        if (err) {
          throw err;
        }
