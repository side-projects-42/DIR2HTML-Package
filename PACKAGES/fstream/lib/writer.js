...
Writer.prototype._create = function () {
  ...
  fs[self.props.follow ? "stat" : "lstat"](self._path, function (er) {
    if (er) {
      return self.warn(
        "Cannot create " + self._path + "\n" + "Unsupported type: " + self.type,
        "ENOTSUP"
      );
    }
    self._finish();
...
Writer.prototype._stat = function (current) {
  ...
  function statCb(er, current) {
    ...
    if (
      ...
      (self.type === "File" && current.nlink > 1)
    ) {
...
function endChown(self, want, current, path, cb) {
  ...
  var chown =
    self.props.follow || self.type !== "SymbolicLink" ? "chown" : "lchown";
...
function endUtimes(self, want, current, path, cb) {
  ...
  var utimes =
    want.follow || self.type !== "SymbolicLink" ? "utimes" : "lutimes";
