...
Compiler.prototype = {
  ...
  prettyIndent: function (offset, newline) {
    ...
    if (this.parentIndents) this.buf.push("buf.push.apply(buf, jade.indent);");
  },
  ...
  visitBlock: function (block) {
    ...
    if (this.parentIndents && block.mode) {
      if (pp)
        this.buf.push(
          "jade.indent.push('" + Array(this.indents + 1).join("  ") + "');"
        );
  ...
  visitAttributes: function (attrs) {
    ...
    if (val.inherits) {
      ...
      this.buffer(
        runtime.attrs(toConstant("{" + val.buf + "}"), JSON.parse(val.escaped))
      );
    } else {
      this.bufferExpression(
